...
"
"               By default we are now (as of 6.0.2) forcing the buffer
...
"               the buffer window. You can set the max height by
...
"               buffer window. You can set the min height by
"               letting the following variable in your .vimrc:
"
...
"               option. This stops the buffer from opening 
...
" When opening a new buffer window, split the new windows below or 
...
" }}}
" Split to edge? {{{
" When opening a new buffer window, split the new windows to the
...
" When sizing the buffer window, assign a maximum window height.
...
" When sizing the buffer window, assign a minumum window height.
...
if g:miniBufExplUseSingleClick == 1
  let s:clickmap = ':if bufname("%") == "buffer" <bar> call <SID>MBEClick() <bar> endif <CR>'
...
function! <SID>StartExplorer(sticky, delBufNum)
  ...
  call <SID>FindCreateWindow('buffer', -1, 1, 1)
...
function! <SID>StopExplorer(sticky)
  ...
  let l:winNum = <SID>FindWindow('buffer', 1)
    ...
    if (a:delBufNum == -1 || l:i != a:delBufNum)
      ...
      if(getbufvar(l:i, '&buflisted') == 1)
        ...
        if(strlen(l:BufName))
          ...
          if (getbufvar(l:i, '&modifiable') == 1 && BufName != 'buffer')
  ...
  if g:miniBufExplorerAutoUpdate == 1
    ...
    if ((g:miniBufExplorerMoreThanOne == 0) || (bufnr('%') != -1 && bufname('%') != ""))
      if <SID>HasEligibleBuffers(a:delBufNum) == 1
        ...
        let l:bufnr = <SID>FindWindow('buffer', 0)
        if (l:bufnr == -1)
...
function! <SID>MBESelectBuffer()
  ...
  if(l:bufnr != -1)             " If the buffer exists.
    ...
    if bufname('%') == 'buffer' || (g:miniBufExplModSelTarget == 1 && getbufvar(bufnr('%'), '&modifiable') == 0)
      wincmd w
      if bufname('%') == 'buffer' || (g:miniBufExplModSelTarget == 1 && getbufvar(bufnr('%'), '&modifiable') == 0)
        wincmd w
        if bufname('%') == 'buffer' || (g:miniBufExplModSelTarget == 1 && getbufvar(bufnr('%'), '&modifiable') == 0)
          wincmd w
          " The following handles the case where buffer
...
function! <SID>MBEDeleteBuffer()
  ...
  if l:selBuf != -1 
    ...
    " Try to get back to the buffer window 
    let l:winNum = bufwinnr(bufnr('buffer'))
    if l:winNum != -1
        exec l:winNum.' wincmd w'
        call <SID>DEBUG('Got to buffer window: '.winnr(),5)
    else
        call <SID>DEBUG('Unable to get to buffer window',1)
    endif
...
"                       the buffer window small when you have lots
...
"               6.0.3 o Changed buffer name to buffer to resolve
